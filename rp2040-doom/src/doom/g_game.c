...
#endif
#if SAVE_COMPRESSED
    printf("G_DoSaveGame() SAVE COMPRESSED\n");
    th_bit_output bo;
    uint32_t size;
    uint8_t *save_buffer = pd_get_work_area(&size);
    save_buffer += 4096; size -= 4096; // we need 4k for flash writing
    printf("G_DoSaveGame() save_buffer=0x%08x size=%d\n", (uint32_t)save_buffer, size);
    sg_bo = &bo;
    th_bit_output_init(sg_bo, save_buffer, size);
    printf("G_DoSaveGame() done setting up buffer\n");
#endif

    savegame_error = false;
    printf("G_DoSaveGame() writing game header\n");
...
#if PICO_ON_DEVICE
    printf("G_DoSaveGame() - saving to slot: %d at 0x%08x\n", savegameslot, (uint32_t)save_buffer);
    if (!P_SaveGameWriteFlashSlot(savegameslot, save_buffer, (int)(bo.cur - save_buffer), save_buffer - 4096)) {
        printf("Not enough space to save game to slot %d", savegameslot);
