...
const scanvideo_timing_t vga_timing_320x240_60_lcd =
        {
            #ifdef ILI9341
                // DO NOT EVER TOUCH THIS VALUE WITH ZAPP'S SIGN OFF OR DEFCON 32 IS IN THE PAST
                .clock_freq = 250000000/20, //Nibbler: This needs to be kept low enough to prevent status bar from glitching - 270,000,000 must be integer multiple of this value
                .h_active = 1280 / 4,
                .v_active = 1000 / 1,
 ...
 */
#ifdef ILI9341
        // Nibbler: Switch to blast processing
        // spi_write_blocking(DISPLAY_SPI_PORT, (uint8_t *)&(buffer->data[2])+1, 2*SCREENWIDTH);
        // Wait for last DMA operation to complete before starting the next
        dma_channel_wait_for_finish_blocking(display_dma_channel);
        dma_channel_configure(display_dma_channel, 
            &channel_config,
            &spi_get_hw(DISPLAY_SPI_PORT)->dr,
            (uint8_t *)&(buffer->data[2])+1,
            2*SCREENWIDTH,
            true
        );
#endif
...
#if PICO_ON_DEVICE
#define LOW_PRIO_IRQ 30  /* Setting this to 31 will break with stdio USB enabled */
...
static void display_write_data(const uint8_t *data, size_t length)
{
 ...
 void display_set_address(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
    ...
    // This is necessary to avoid contention with DMA transfers
    busy_wait_ms(2);
...
// Get the processor sys_clk frequency in Hz
 ...
 clock_configure(clk_peri,
    ...
    // SPI Display DMA init
    printf("display_spi_master_init() Setting up DMA\n");
    display_dma_channel = 7;
    printf("display_spi_master_init() Using DMA channel: %d\n", display_dma_channel);
    channel_config = dma_channel_get_default_config(display_dma_channel);
    channel_config_set_transfer_data_size(&channel_config, DMA_SIZE_8);
    printf("display_spi_master_init() Setting up SPI dreq\n");
    channel_config_set_dreq(&channel_config, spi_get_dreq(DISPLAY_SPI_PORT, true));
